...
function getSong(iSongId) {
  return songs
    ...
    .map(song => {
      song.recordingsHistory = recordings.filter(
        recording =>
          recording.songs.filter(
            oSong => parseInt(oSong.linkid, 10) === parseInt(iSongId, 10)
          ).length > 0
      );
      return song;
    })[0];
...
function App() {
  ...
  function generateSearchResults() {
    ...
    function formattedSearchResult(match, sPrefix) {
      return (
        (sPrefix || "") + match.replace(re, '<span class="highlight">$1</span>')
      );
    }
  ...
  function determineRandomPath() {
    if (Math.round(Math.random() * 10) > 1) {
      ...
      return (
        "/recordings/" +
        recordings[Math.round(Math.random() * recordings.length) + 1].linkid
      );
    } else {
      return (
        "/songs/" + songs[Math.round(Math.random() * songs.length) + 1].linkid
      );
    }
  }
  ...
  return (
    <BrowserRouter basename={process.env.PUBLIC_URL}>
              ...
              <div className="recordings">
                ...
                <ul>
                  {recordings
                    ...
                    .map((o, i) => (
                      <li key={i}>
                        <Link to={"/recordings/" + o.linkid}>
                          {getFormattedRecording(o)}
                        </Link>
                      </li>
                    ))}
                </ul>
                <div className="bottomText">
                  <p>
                    The majority of the data for this website came from{" "}
                    <a href="http://www.bobsnerdywebpage.com/">
                      Bob&apos;s Nerdy Website
                    </a>
                  </p>
                  <p>
                    Raw data:{" "}
                    <a href="https://github.com/jonbri/icecoldnugrape/blob/master/src/recordings_raw.json">
                      recordings_raw.json
                    </a>
                    ,{" "}
                    <a href="https://github.com/jonbri/icecoldnugrape/blob/master/src/songs_raw.json">
                      songs_raw.json
                    </a>
                  </p>
                </div>
        ...
        render={props => (
          <div className="app">
            <Header
              ...
              previousRecording={
                recordings[
                  determineRecordingIndex(props.match.params.recordingId) - 1
                ]
              }
              nextRecording={
                recordings[
            ...
            <div className="app-body">
              <div className="recording">
                <h3>
                  {getFormattedRecording(
                    getRecording(props.match.params.recordingId)
                  )}
                </h3>
                <ol>
                  {getRecording(props.match.params.recordingId).songs.map(
                    (o, i) => (
                      <li key={i}>
                        <Link to={"/songs/" + o.linkid}>
                          {i + 1}. {getSong(o.linkid).value}
                        </Link>
                      </li>
                    )
                  )}
                ...
                {(getRecording(props.match.params.recordingId).quality ||
                  getRecording(props.match.params.recordingId).comments.length >
                    0) && (
                  <div className="subcontent">
                    {getRecording(props.match.params.recordingId).quality && (
                      <div className="recording-metadata">
                        <label>Best known quality:</label>&nbsp;
                        {getRecording(props.match.params.recordingId).quality}
                      </div>
                    )}
                    ...
                    {getRecording(props.match.params.recordingId).comments
                      ...
                      <ul>
                        {getRecording(
                          props.match.params.recordingId
                        ).comments.map((o, i) => (
                          <li key={i}>
                            <header>
                              {o.name + " (" + o.time.split(" ")[0] + ")"}
                            </header>
                            {o.text}
                          </li>
                        ))}
            ...
            <div className="app-body">
              <div className="songs">
                ...
                <ul>
                  {songs.map((o, i) => (
                    <li key={i}>
                      <Link to={"/songs/" + o.linkid}>{o.value}</Link>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="app">
            ...
            <div className="app-body">
              <div className="song">
                ...
                <ul>
                  {getSong(props.match.params.songId).recordingsHistory.map(
                    (o, i) => (
                      <li key={i}>
                        <Link to={"/recordings/" + o.linkid}>
                          {getFormattedRecording(o)}
                ...
                {getSong(props.match.params.songId).comments.length > 0 && (
                  <div className="subcontent">
                    <ul>
                      {getSong(props.match.params.songId).comments.map(
                        (o, i) => (
                          <li key={i}>
                            <header>
                              {o.name + " (" + o.time.split(" ")[0] + ")"}
            ...
            <div className="app-body">
              <div className="downloads">
                ...
                {downloads_raw.map((oDownload, i) => (
                  <ul key={i}>
                    <li>
                      {oDownload.title !== "" && <h3>{oDownload.title}</h3>}

                      {oDownload.zip && (
                        <a
                          className="zip"
                          href={
                            "/media/" + oDownload.group + "/" + oDownload.zip
                          }
                        >
                          Download All
                        </a>
                      ...
                      {oDownload.songs.map((oSong, j) => (
                        <ul key={j}>
                          <li>
                            <a
                              href={
                                "/media/" + oDownload.group + "/" + oSong.path
                              }
                            >
                              {oSong.title}
                            </a>
      ...
      <Route
        exact
        path="/random"
        render={() => <Redirect to={determineRandomPath()} />}
      />
